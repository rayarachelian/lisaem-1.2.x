...
HQX_API void HQX_CALLCONV hq3x_32_rb( int rowbytes,  uint32 srb,   wxBitmap *mybitmap, uint32 drb, int Xres, int Yres, uint32 brightness )
{
    ...
    //int   dpL = (drb >> 2);               // destination next line?
    //uint8 *sRowP = (uint8 *) &sp;           // <- :TODO: recheck these!!! might not be right, mgiht need to be a pointer to data
    //uint8 *dRowP = (uint8 *) &dp;           // -- we replace this with *Data which is PixelData not uint8
    uint32 yuv1, yuv2;
...
// j=y coordinate, i=x coordinate.
    ...
    #define GETDOUBLEPIXEL(x,ptr) (  (double_video_bits[ptr[(x>>4)         ]] & ( (1<<(15-(x & 15))) )) ? 0: bright  )
    #define GETSINGLEPIXEL(x,ptr) (  (                  ptr[(x>>3)          ] & ( (1<<(7 -(x &  7))) )) ? 0: bright  )
    ...
    for (y=0; y<Yres; y++)
    {
        ...
        for (x=0; x<Xres; x++)
        {
           ...
           // and the X==0 X<Xres-1 checks - possibly premature optimization, but might be doable.
            w[2] = GETSINGLEPIXEL(x,up); //(*(sp + prevline) & (1<<(7-((x  )&7) ))) ? bright:0x00;      // w[2] = *(sp + prevline);
            w[5] = GETSINGLEPIXEL(x,sp); //(*(sp           ) & (1<<(7-((x  )&7) ))) ? bright:0x00;      // w[5] = *sp;
            w[8] = GETSINGLEPIXEL(x,dn); // (*(sp + nextline) & (1<<(7-((x  )&7) ))) ? bright:0x00;      // w[8] = *(sp + nextline);

            if (x>0) // 1,4,7 are the left column
            {
                w[1] = GETSINGLEPIXEL(x-1,up); //(*(sp + prevline) & (1<<(7-((x-1)&7) ))) ? bright:0x00;  //*(sp + prevline - 1);
                w[4] = GETSINGLEPIXEL(x-1,sp); //(*(sp           ) & (1<<(7-((x-1)&7) ))) ? bright:0x00;  //*(sp - 1);
                w[7] = GETSINGLEPIXEL(x-1,dn); //(*(sp + nextline) & (1<<(7-((x-1)&7) ))) ? bright:0x00;  //*(sp + nextline - 1);
            }
            else
            {
                w[1] = w[2];
            ...
            if (x<Xres-1)
            {
                w[3] = GETSINGLEPIXEL(x+1,up); //*(sp + prevline + 1);
                w[6] = GETSINGLEPIXEL(x+1,sp); //*(sp + 1);
                w[9] = GETSINGLEPIXEL(x+1,dn); //*(sp + nextline + 1);
            }
            else
            {
                w[3] = w[2];
...
HQX_API void HQX_CALLCONV hq3x_32(wxBitmap *mybitmap, int Xres, int Yres, int brightness )
{
...
// HQX_API void HQX_CALLCONV hq3x_32_rb(       uint8 * sp, uint32 srb,     uint32 * dp, uint32 drb,       int Xres, int Yres, uint32 brightness )
        hq3x_32_rb( 90, rowBytesL,mybitmap, rowBytesL * 3,    Xres,     Yres, brightness);

}

HQX_API void HQX_CALLCONV hq3x_32_3a(wxBitmap *mybitmap, int Xres, int Yres, int brightness )
{
    uint32 rowBytesL = Xres * 4;                            // *** here here here //
// HQX_API void HQX_CALLCONV hq3x_32_rb(       uint8 * sp, uint32 srb,     uint32 * dp, uint32 drb,       int Xres, int Yres, uint32 brightness )
        hq3x_32_rb( 76, rowBytesL,mybitmap, rowBytesL * 3,    Xres,     Yres, brightness);
